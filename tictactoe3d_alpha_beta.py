...
Original file is located at
    https://colab.research.google.com/drive/10L-Yo58ynHKJDYxRbOgvclsLwtBlyyJJ
...
In this implementation we define two AI classes (AI1 and AI2) that use alpha–beta search with forward pruning. Each AI uses a different evaluation (heuristic) function:

• Heuristic 1 (Exponential “potential‐lines” evaluation):
 – For every winning “line” (a contiguous set of 5 cells in any straight direction), if the line is “open” (i.e. not blocked by the opponent) it scores an exponential value (10^n where n is the number of the AI’s pieces in that line). Similarly, opponent lines subtract an exponential value.
 – This heuristic aggressively rewards positions near a win.

• Heuristic 2 (Linear “potential‐lines” evaluation):
 – The same “open line” concept is used, but here each line scores a simple linear value (n for n pieces) and subtracts n for the opponent.
 – This leads to a more “conservative” evaluation.

Both AIs use a generic alpha–beta search that first orders the legal moves (using a one–ply lookahead with the respective evaluation) and then prunes the move list to a fixed number (forward pruning) before deeper search. We also include a helper function to “clone” the game state so that search simulations do not interfere with the actual game. Finally, we simulate 100 games on a 5×5×5 board (where five in a row wins) and print the results, along with an analysis of the outcome.
"""
